...
describe('foodSmeltingBehavior', () => {
  let foodCollection: FoodCollectionHandle;
  let handle: FoodSmeltingHandle;

  beforeEach(() => {
    jest.clearAllMocks();
    jest.setSystemTime(0);
    foodCollection = createFoodCollectionBehavior();
    foodCollection.resetCooldown();
    handle = createFoodSmeltingBehavior({ foodCollection });
  ...
  describe('shouldActivate', () => {
    ...
    it('respects cooldown after planning failure', async () => {
      ...
      const state = await handle.behavior.createState(bot);
    ...
    it('returns true when food points below trigger but food collection is in cooldown', () => {
      ...
      const bot = createBotWithRawFood({ beef: 3 });  // 9 points < trigger (10)
      foodCollection.triggerCooldown();
      expect(handle.behavior.shouldActivate(bot)).toBe(true);
    });
  ...
  describe('priority', () => {
    it('has priority 40 (lower than food_collection at 60)', () => {
      expect(handle.behavior.priority).toBe(40);
    });

    it('has correct name', () => {
      expect(handle.behavior.name).toBe('food_smelting');
    });
  });
