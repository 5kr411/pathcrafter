const { hoistMiningInPath, hoistMiningInPaths } = require('../../path_optimizations/hoistMining');
...
describe('unit: hoist mining optimizer', () => {
    ...
    test('separates mining steps by tool key', () => {
        ...
        const out = hoistMiningInPath(path);
        const wood = out.find(s => s.action === 'mine' && s.tool === 'wooden_pickaxe');
        const stone = out.find(s => s.action === 'mine' && s.tool === 'stone_pickaxe');
    ...
    test('handles targetItem key variant', () => {
        ...
        expect(out[0].targetItem).toBe('generic_log');
