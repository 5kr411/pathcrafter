...
const { captureAdaptiveSnapshot, createPathValidator } = require('../../utils/adaptiveSnapshot');
...
describe('integration: adaptive snapshot with progressive radii', () => {
  ...
  function createMockBot(fullSnapshot) {
    return {
      version: '1.20.1',
      entity: {
        position: {
      ...
      findBlocks: function({ matching, maxDistance }) {
        // Return blocks within maxDistance
        const positions = [];
        const center = fullSnapshot.center;
        
        for (const [blockName, stats] of Object.entries(fullSnapshot.blocks || {})) {
  ...
  test('progressively increases radius when small radius lacks resources', async () => {
    ...
    const validator = async (snapshot) => {
