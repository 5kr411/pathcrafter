...
describe('integration: workstation inventory shortcut', () => {
  ...
  test('does not re-craft crafting_table when already in inventory', () => {
    const inventory = new Map([['crafting_table', 1], ['stick', 2], ['oak_planks', 10], ['stone_pickaxe', 1]]);
    const snapshot = {
    ...
    const tree = plan(mcData, 'iron_pickaxe', 1, { log: false, inventory, worldSnapshot: snapshot, pruneWithWorld: true });
