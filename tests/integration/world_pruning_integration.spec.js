...
const { countActionPaths } = require('../../action_tree/metrics')
...
describe('integration: world-pruning planning (generic wood disabled)', () => {
  ...
  test('prunes mining source when world has no coal_ore', () => {
    ...
    const noCoal = { version: '1.20.1', dimension: 'overworld', center: { x:0,y:64,z:0 }, chunkRadius: 1, yMin: 0, yMax: 255, blocks: { }, entities: {} }
    ...
    const hasMiningCoal = lwPruned.some(seq => seq.some(s => s.action === 'mine' && (s.targetItem === 'coal' || s.what === 'coal_ore')))
  ...
  test('allows mining when world has enough coal_ore', () => {
    ...
    const hasCoal = { version: '1.20.1', dimension: 'overworld', center: { x:0,y:64,z:0 }, chunkRadius: 1, yMin: 0, yMax: 255, blocks: { coal_ore: { count: 5, closestDistance: 10, averageDistance: 12 } }, entities: {} }
    ...
    const hasMiningCoal = lwOk.some(seq => seq.some(s => s.action === 'mine' && (s.targetItem === 'coal' || s.what === 'coal_ore')))
