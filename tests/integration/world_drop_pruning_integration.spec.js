...
describe('integration: world pruning respects block->drop mapping', () => {
  ...
  test('coal target considers coal_ore availability', () => {
    ...
    const snap = { version: '1.20.1', dimension: 'overworld', center: { x:0,y:64,z:0 }, chunkRadius: 1, yMin: 0, yMax: 255, blocks: { coal_ore: { count: 3, closestDistance: 8 } }, entities: {} }
    ...
    const hasMining = paths.some(seq => seq.some(s => s.action === 'mine' && (s.targetItem === 'coal' || s.what === 'coal_ore')))
    expect(hasMining).toBe(true)
  })

  test('raw_iron target considers iron_ore availability', () => {
    const inventory = { stone_pickaxe: 1 }
    const snap = { version: '1.20.1', dimension: 'overworld', center: { x:0,y:64,z:0 }, chunkRadius: 1, yMin: 0, yMax: 255, blocks: { iron_ore: { count: 4, closestDistance: 10 }, deepslate_iron_ore: { count: 2, closestDistance: 12 } }, entities: {} }
    const tree = plan(ctx, 'raw_iron', 3, { log: false, inventory, pruneWithWorld: true, worldSnapshot: snap })
    const paths = Array.from(enumerateLowestWeightPathsGenerator(tree, { inventory }))
    const hasMining = paths.some(seq => seq.some(s => s.action === 'mine' && (s.targetItem === 'raw_iron' || s.what === 'iron_ore' || s.what === 'deepslate_iron_ore')))
    expect(hasMining).toBe(true)
  })

  test('prunes when snapshot lacks matching source blocks for target drop', () => {
    const inventory = { stone_pickaxe: 1 }
    const snap = { version: '1.20.1', dimension: 'overworld', center: { x:0,y:64,z:0 }, chunkRadius: 1, yMin: 0, yMax: 255, blocks: { }, entities: {} }
