...
class BehaviorGetClosestEntity {
    ...
    getClosestEntity() {
        ...
        if (closest) {
            ...
            const itemId = closest.metadata[8]?.itemId;
            const itemName = itemId ? mcData.items[itemId]?.name : 'unknown';
            console.log(`BehaviorGetClosestEntity: closest: ${itemName} (ID: ${itemId})`);
        } else {
            console.log('BehaviorGetClosestEntity: closest: null');
        }
        return closest;
    }
}
