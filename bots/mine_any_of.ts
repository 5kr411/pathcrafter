...
bot.once('spawn', () => {
  ...
  const mine = createMineAnyOfState(bot, targets);
  const exit = new BehaviorIdle();

  const start = new StateTransition({
    name: 'mine-any-of: enter -> mine',
    parent: enter,
    child: mine,
  ...
  root.name = 'mine_any_of_root';

  bot.on('chat', (username: string, message: string) => {
    if (username === bot.username) return;
    const m = message.trim();
    const parts = m.split(/\s+/);
    if (parts[0] !== 'mineany') return;
    const amount = parseInt(parts[1]);
    const names = parts.slice(2);
    if (!Number.isFinite(amount) || names.length === 0) {
      try {
        bot.chat('usage: mineany <amount> <block1> [block2] ...');
      } catch (_) {}
      return;
    }
