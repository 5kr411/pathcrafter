...
function isPathValidBasic(path, initialSupply, getSmeltsPerUnitForFuel) {
    ...
    for (const st of path) {
        ...
        if (st.action === 'craft') {
            if (st.what === 'table') { const haveTable = (supply.get('crafting_table') || 0) > 0; if (!haveTable) return false; }
            ...
            const resItem = st.result?.item; const resCount = (st.result?.perCraftCount || 1) * (st.count || 1); add(resItem, resCount); continue;
        }
        if (st.action === 'smelt') {
            const haveFurnace = (supply.get('furnace') || 0) > 0; if (!haveFurnace) return false;
...
function isPathValidWithFamilies(path, initialSupply, getSmeltsPerUnitForFuel) {
    const supply = new Map(initialSupply);
    let needTableFamily = false;
    let needFurnaceFamily = false;
