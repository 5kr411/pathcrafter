...
function buildRecipeTree(ctx, itemName, targetCount = 1, context = {}) {
    ...
    recipes.forEach(recipe => {
        ...
        const craftNode = {
            ...
            result: { item: itemName, perCraftCount: recipe.result.count, meta: { generic: (baseForcedGeneric && getGenericWoodEnabled()) || false, selectedSpecies: baseForcedGeneric ? null : (resultSpecies || null) } },
            ingredients: Array.from(ingredientCounts.entries()).sort(([a], [b]) => a - b).map(([id, count]) => {
                ...
                return { item: ingName, perCraftCount: count, meta: { generic: genericAllowed && ((isFamily && !selectedSpecies && preferWoodFamilies) || familyGenericBases.has(base)), selectedSpecies } };
            }),
            children: []
        };
