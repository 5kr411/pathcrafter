...
function createMineOneOfState(bot: Bot, targets: Targets): any {
  ...
  function selectBestCandidate(): { blockName: string; itemName: string; amount: number } | null {
    ...
    const required = Number(targets.amount || (list[0] && list[0].amount) || 1);
    ...
    if (!best) {
      ...
      for (const c of list) {
        const evalRes = evaluateCandidate(c.blockName, required);
        if (
          (evalRes.count || 0) > bestCount ||
          ((evalRes.count || 0) === bestCount && evalRes.nearest < bestNear)
        ) {
  ...
  const tPrepareToExit = new StateTransition({
    parent: prepare,
    child: exit,
    name: 'mine-one-of: prepare -> exit (no selection)',
    shouldTransition: () => !selection || !selection.chosen,
    onTransition: () => {
      try {
        logger.error('BehaviorMineOneOf: no viable candidate found; exiting');
      } catch (_) {}
    }
  });
  ...
  const tCollectToExit = new StateTransition({
    ...
    child: exit,
    name: 'mine-one-of: collect -> exit',
    shouldTransition: () =>
      typeof collectBehavior.isFinished === 'function' ? collectBehavior.isFinished() : true
  });
