const { BehaviorIdle, StateTransition, NestedStateMachine } = require('mineflayer-statemachine');
...
function createAcquireCraftingTableState(bot: Bot, targets: Targets = {}): any {
  ...
  const collectLogsToExit = new StateTransition({
    ...
    child: exit,
    shouldTransition: () =>
      collectLogsIfNeededState.isFinished() && getItemCountInInventory(bot, 'oak_log') < 1,
    onTransition: () => {
  ...
  const craftPlanksToExit = new StateTransition({
    ...
    child: exit,
    shouldTransition: () =>
      craftPlanksIfNeededState.isFinished() && getItemCountInInventory(bot, 'oak_planks') < 4,
    onTransition: () => {
...
export default createAcquireCraftingTableState;
