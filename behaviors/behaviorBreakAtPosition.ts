...
function createBreakAtPositionState(bot: Bot, targets: Targets): any {
  ...
  addStateLogging(moveTo, 'MoveTo', {
    ...
    getExtraInfo: () => {
      ...
      if (!botPos || !botPos.distanceTo) return `to break position (${pos.x}, ${pos.y}, ${pos.z})`;
  ...
  let loggingMineStartTime: number | null = null;
  const originalMineOnStateEntered =
    typeof mine.onStateEntered === 'function' ? mine.onStateEntered.bind(mine) : null;
  ...
  const originalMineOnStateExited =
    typeof mine.onStateExited === 'function' ? mine.onStateExited.bind(mine) : null;
  ...
  function waitForBlockToBecomeAir(pos: Vec3Like | null | undefined, timeoutMs: number = 8000): Promise<boolean> {
    try {
    ...
    return new Promise((resolve) => {
      ...
      let done = false;
      const onUpdate = (_oldBlock: Block, newBlock: Block) => {
        if (done) return;
      ...
      const cleanup = () => {
        try {
          bot.world!.removeListener(eventName, onUpdate);
        } catch (_) {}
  ...
  const moveToMine = new StateTransition({
    ...
    onTransition: () => {
      ...
      const pos =
        targets.blockPosition && targets.blockPosition.clone
          ? targets.blockPosition.clone()
          : targets.blockPosition;
      waitForBlockToBecomeAir(pos, 10000).then((ok) => {
        if (ok) brokenObserved = true;
      });
    }
  });
  ...
  const moveToExit = new StateTransition({
    ...
    onTransition: () => {
      ...
      logger.info(
        `BehaviorBreakAtPosition: move -> exit (elapsed=${elapsed}ms, dist=${
          moveTo.distanceToTarget && moveTo.distanceToTarget()
        })`
      );
    }
  });

  let mineFinishTime: number | null = null;
  ...
  const mineToExit = new StateTransition({
    ...
    onTransition: () => {
      ...
      logger.info(
        `BehaviorBreakAtPosition: mine -> exit (move took ${moveDuration}ms, mine took ${mineDuration}ms)`
      );
    }
  });
  ...
  const mineToRetry = new StateTransition({
    ...
    shouldTransition: () => {
      try {
        const stillThere =
          targets.blockPosition && bot.world!.getBlockType(targets.blockPosition) !== 0;
        return mine.isFinished && !brokenObserved && stillThere && digRetries < 4;
      } catch (_) {
        return mine.isFinished && !brokenObserved && digRetries < 4;
      }
    },
    onTransition: () => {
      digRetries++;
