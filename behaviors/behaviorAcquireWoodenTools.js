...
function createAcquireWoodenToolsState(bot, targets) {
    ...
    function needToCollectLogs(bot) {
        ...
        if (hasWoodenTool(bot) && getItemCountInInventory(bot, 'oak_planks') >= 5) {
            logger.info('BehaviorAcquireWoodenTools: do not need to collect logs: already have a wooden tool and 5 planks')
            return false
        }

        if (hasWoodenTool(bot) && getItemCountInInventory(bot, 'oak_planks') >= 3 && getItemCountInInventory(bot, 'stick') >= 2) {
            logger.info('BehaviorAcquireWoodenTools: do not need to collect logs: already have a wooden tool and 3 planks and 2 sticks')
            return false
        }
